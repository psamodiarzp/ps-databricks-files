...
WITH cte AS(
 ...
 --- inner join batch_sheets.checkout_v2_rampup_sheet b on a.merchant_id = b.merchant_id
  WHERE
    ...
    and checkout_id <> '00000000000000'
...
cte2 AS(
  with payments_cte as(
    select id, method, authorized_at
...
p.authorized_at,
  ...
  inner join ( Select checkout_id,payment_id  from realtime_hudi_api.payment_analytics
  WHERE created_date between ('{0}') and ('{1}') 
...
select *, coalesce(submit_checkout_id,ps_checkout_id,pa_checkout_id) as modified_submit_checkout_id
from cte3
...
# Create a flag column 'Matched' and set it to False by default
raw_data_df['experiment'] = 'false'

# Iterate over the ID column and set 'Matched' to True if the ID is in the list
raw_data_df.loc[raw_data_df['merchant_id'].isin(final_mid_list), 'experiment'] = 'true'
...
#Modified submit logic is to account for the cases where payment ID or successful are present but not submit.
#raw_data_df['modified_submit_checkout_id'] = raw_data_df.apply(lambda row: row['submit_checkout_id'] or row['pa_checkout_id'] or row['ps_checkout_id'], axis=1)

# COMMAND ----------

overall_df = raw_data_df.groupby(by = ['v2_result','v2_eligible','experiment'], dropna=False).agg(
    { 'checkout_id':'nunique',
        'merchant_id':'nunique',
     ...
     'modified_submit_checkout_id':'nunique',
...
overall_df['modified_modal_cr'] = overall_df['modified_submit_checkout_id']*1.0 / overall_df['checkout_id']
...
overall_df['period'] = start_date + " to " + end_date

# COMMAND ----------

overall_df.to_csv('/dbfs/FileStore/overall_df.csv', index=False)
"https://razorpay-dev.cloud.databricks.com/files/overall_df.csv"

# COMMAND ----------

#checking submit to pa issue
submit_to_pa_cid = raw_data_df.groupby(by = ['v2_result','v2_eligible','experiment','producer_created_date','checkout_id'], dropna=False).agg(
    { 
        'submit_checkout_id':'nunique',
     'pa_checkout_id':'nunique',
        'ps_checkout_id':'nunique',
...
submit_to_pa_cid['submit_to_pa'] = round(submit_to_pa_cid['payment_attempts']*1.0 / submit_to_pa_cid['submit_checkout_id'],4)
submit_to_pa_combined = submit_to_pa_cid.groupby(by = ['v2_result','v2_eligible','experiment','producer_created_date','submit_to_pa'], dropna=False).agg(
{'checkout_id':'nunique'}
)
...
submit_to_pa_cid[submit_to_pa_cid['submit_to_pa'] == 756608]

# COMMAND ----------

raw_data_df[raw_data_df['checkout_id']=='00000000000000']

# COMMAND ----------

submit_to_pa_combined = submit_to_pa_combined.reset_index()

# COMMAND ----------

submit_to_pa_combined.to_csv('/dbfs/FileStore/submit_to_pa_combined.csv', index=False)
"https://razorpay-dev.cloud.databricks.com/files/submit_to_pa_combined.csv"

# COMMAND ----------

submit_to_pa_combined.sort_values(by='checkout_id', ascending=False)

# COMMAND ----------

# MAGIC %md
# MAGIC ## Appending to existing data

# COMMAND ----------

appended_df = pd.read_csv('/dbfs/FileStore/std_cr_breakdown_june_sept_df.csv')
appended_df.head()

# COMMAND ----------

appended_df = pd.concat([appended_df, overall_df])
appended_df.head()
...
appended_df.to_csv('/dbfs/FileStore/std_cr_breakdown_june_sept_df.csv', index=False)

# COMMAND ----------

"https://razorpay-dev.cloud.databricks.com/files/std_cr_breakdown_june_sept_df.csv"

# COMMAND ----------

# MAGIC %md
# MAGIC ## Compiled Results
...
"""
email_id = ['pallavi.samodia@razorpay.com','sanjay.garg@razorpay.com','aakash.bhattacharjee@razorpay.com','manjeet.singh@razorpay.com','chirag.patel@razorpay.com','aravinth.pk@razorpay.com','kruthika.m@razorpay.com','revanth.m@razorpay.com','pranav.gupta@razorpay.com','ankit.punia@razorpay.com','gaurav.dadhich@razorpay.com','shivam.bansal@razorpay.com','vishal.gupta@razorpay.com']
# 
#for i in email_id:
...
# MAGIC where producer_created_date >= date('2024-09-01')
# MAGIC group by 1

...
select *, 
case when  mid_insurance.mid is not null then 1 else 0 end as pricing_merchant
from payments
left join fulfillment on payments.order_id = fulfillment.order_id
LEFT JOIN batch_sheets.magic_checkout_rto_insurance as mid_insurance on payments.merchant_id = mid_insurance.mid
...
where   (payments.payment_date >= rto_insurance_start_date 
  ...
  or (mid_insurance.mid is null)  --- add filter for payment date + cod intelligence or manual is not null
--limit 1000
""")
...
tracker_details_df[(tracker_details_df['merchant_id']=='E2NWmnnFI68dlm') & (tracker_details_df['payment_month_year']=='2023-12')
                   # & (tracker_details_df['shipping_status']=='rto')
                   ]

# COMMAND ----------

if tracker_details_df['pricing_merchant']==1:
...
tracker_summary_new_df = tracker_details_df.groupby(['payment_month_year', 'merchant_id']).agg(
    ...
    name=('name', 'max'),
    pricing_merchant=('pricing_merchant', 'max'),
).reset_index()
...
tracker_summary_df[tracker_summary_df['merchant_id'] == 'E2NWmnnFI68dlm']
...
user = 'santosh.taduru@razorpay.com'
app_password = 'lgfvclhofxqbescq' # Guide for app passwords: https://support.google.com/accounts/answer/185833?hl=en
host = 'smtp.gmail.com'
port = 465
...
tracker_summary_df[(tracker_summary_df['email_cc'] =='') & (tracker_summary_df['invoice_status'] =='Success')]
...
send_emails(datetime.now().strftime('%Y-%m'), 'All Merchants', tracker_summary_df,'RTO Reimbursement Tracker Summary', 0,'magic-rto@razorpay.com','pallavi.samodia@razorpay.com','Razorpay')
            #tracker_summary_df[tracker_summary_df['rto_invoiced']==datetime.now().strftime('%Y-%m-%d')]['rto_amount'].astype('float').sum())

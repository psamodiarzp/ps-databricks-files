...
def send_emails(start_date, end_date,email_id_list,html_t):
    ...
    subject = 'October Magic Checkout V2 vs V1 report [without Deodap] - {0} to {1}'.format(start_date, end_date,)
...
<html>
<head>
    <title>[October sans Deodap] Magic Checkout V2 vs V1 report - {0} to {1} </title>
    <style>
    td, th {{
...
case when id is not null  then cte.checkout_id else null end as pa_cid,
case when authorized_at is not null or cte2.method='cod' then cte.checkout_id else null end as ps_cid,
...
#write an either/or
conditions = ((final_df['v2_eligible']=='true') |(final_df['v2_eligible']==True)) & (final_df['merchant_id'] != 'ElKk2sBD1gFHvw') & (final_df['merchant_id'] != 'Ba2to8xoI5kO2x') & (final_df['merchant_id'] != 'FQLy8isl8cBT4q')
...
(CASE
     ...
     WHEN event_name='render:complete' THEN '1.01 Render Complete'
      ...
      count(distinct checkout_id) as cnt_checkout_id
...
pa_rows = pd.DataFrame({'v2_result': final_l0_df_copy['v2_result'],
                         ...
                         'cnt_checkout_id': final_l0_df_copy['payment_attempts']})    
ps_rows = pd.DataFrame({'v2_result': final_l0_df_copy['v2_result'],
                         'v2_eligible': True,
                         'funnel_steps': '7.2 payment_success',
                         'cnt_checkout_id': final_l0_df_copy['payment_success']})                                                  
...
html_code += """<p><h3>New Address Funnel:</h3> <br/>"""
...
SELECT
    v2_result,
   -- v2_eligible,
    COUNT(IF(payment_screen_loaded = 1, checkout_id, NULL)) AS payment_screen_loaded,
    
    COUNT(IF(payment_screen_loaded = 1 AND cod_shown = 1, checkout_id, NULL)) AS cod_shown,
    COUNT(IF(payment_screen_loaded = 1 AND cod_selected = 1, checkout_id, NULL)) AS cod_selected,
 ---   COUNT(IF(cod_L1_screen_shown = 1, checkout_id, NULL)) AS cod_L1_screen_shown,
    COUNT(IF(cod_submit = 1, checkout_id, NULL)) AS cod_submit,

  ---  COUNT(IF(payment_screen_loaded = 1 AND upi_shown = 1, checkout_id, NULL)) AS upi_shown,
    COUNT(IF(payment_screen_loaded = 1  AND upi_selected = 1, checkout_id, NULL)) AS upi_selected,
 ---   COUNT(IF(upi_l1_screen_shown = 1, checkout_id, NULL)) AS upi_l1_screen_shown,
    COUNT(IF(upi_submit = 1, checkout_id, NULL)) AS upi_submit,

  ---  COUNT(IF(payment_screen_loaded = 1 AND card_shown = 1, checkout_id, NULL)) AS card_shown,
    COUNT(IF(payment_screen_loaded = 1 AND card_selected = 1, checkout_id, NULL)) AS card_selected,
  ---  COUNT(IF(card_l1_screen_shown = 1, checkout_id, NULL)) AS card_l1_screen_shown,
    COUNT(IF(card_submit = 1, checkout_id, NULL)) AS card_submit,

  ---  COUNT(IF(payment_screen_loaded = 1 AND emi_shown = 1, checkout_id, NULL)) AS emi_shown,
    COUNT(IF(payment_screen_loaded = 1  AND emi_selected = 1, checkout_id, NULL)) AS emi_selected,
  ---  COUNT(IF(emi_l1_screen_shown = 1, checkout_id, NULL)) AS emi_l1_screen_shown,
    COUNT(IF(emi_submit = 1, checkout_id, NULL)) AS emi_submit,

  --  COUNT(IF(payment_screen_loaded = 1 AND netbanking_shown = 1, checkout_id, NULL)) AS netbanking_shown,
    COUNT(IF(payment_screen_loaded = 1 AND netbanking_selected = 1, checkout_id, NULL)) AS netbanking_selected,
 ---   COUNT(IF(netbanking_l1_screen_shown = 1, checkout_id, NULL)) AS netbanking_l1_screen_shown,
    COUNT(IF(netbanking_submit = 1, checkout_id, NULL)) AS netbanking_submit,

    COUNT(IF(other_prepaid_selected = 1, checkout_id, NULL)) AS other_prepaid_selected,
 ---   COUNT(IF(other_prepaid_l1_screen_shown = 1, checkout_id, NULL)) AS other_prepaid_l1_screen_shown,
    COUNT(IF(other_prepaid_submit = 1, checkout_id, NULL)) AS other_prepaid_submit,
    COUNT(IF(submit = 1, checkout_id, NULL)) AS submit
...
html_code += """<p><h3>Payment Screen Funnel:</h3> <br/>"""
...
# MAGIC %md
# MAGIC ## Browser-wise SR deep dive
...
# 
#for i in email_id:
email_id=['pallavi.samodia@razorpay.com','sanjay.garg@razorpay.com','aakash.bhattacharjee@razorpay.com','manjeet.singh@razorpay.com','chetna.handa@razorpay.com']

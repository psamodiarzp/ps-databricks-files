...
and merchant_id='CEJeWFPkKwBRUI'
...
json_array_length(get_json_object(value, '$.line_items')) AS sku
 ...
 and type='one_click_checkout'
  ...
  and order_meta.created_date>='2024-01-01'
...
select payments.order_id, 
 ...
 = 'no') AND (NOT (payments.authorized_at IS NULL))) THEN 'payment_successful' ELSE NULL END) AS orders_placed,
   round(SUM(CASE WHEN (CASE WHEN payments.method = 'cod' THEN 'yes' ELSE 'no' END
 = 'yes') OR ((CASE WHEN payments.method = 'cod' THEN 'yes' ELSE 'no' END
 = 'no') AND (NOT (payments.authorized_at IS NULL))) THEN payments.base_amount  /100.00  ELSE NULL END),2) AS GMV
 ...
 and date(payments.created_date) >= (date_add(date(now()), -1))
  ...
  /*
   ...
   where order_meta.created_date >= '2024-01-01'
  and payments.created_date >= '2024-01-01' 
  ...
  */
  ...
  and payments.merchant_id = 'CEJeWFPkKwBRUI'
...
get_json_object(value, '$.utm_parameters.utm_content') AS utm_content,
json_array_length(get_json_object(value, '$.line_items')) AS sku,
round(COALESCE(GMV, 0),2) as Order_GMV,
COALESCE(orders_placed, 0) as orders_placed 
from payments 
...
app_password = 'udyqypqzxkjuwycf' # Guide for app passwords: https://support.google.com/accounts/answer/185833?hl=en
host = 'smtp.gmail.com'
port = 465
...
att_name_label = ['Gir_hourly_report_']
subject_label = ['Hourly Report from Razorpay Magic Checkout: Gir',]

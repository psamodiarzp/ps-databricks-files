...
def send_emails(start_date, end_date,email_id_list,html_t):
    ...
    subject = 'Quickbuy report - {0} to {1}'.format(start_date, end_date,)
...
<html>
<head>
    <title>Quickbuy report - {0} to {1} </title>
    <style>
    td, th {{
...
<body>
<h2> Folks, this report is a rendition of the V2 report so v2_result : quickbuy experiment result and v2_eligible: quickbuy_eligible </h2>
...
WITH cte AS(
  ...
  ---(ramp_up_date),
   ...
   (get_json_object(render_properties,'$.data.meta.quickbuy_eligible')) as v2_eligible,
 ...
 select_section,
  ...
  FROM
    ...
    and exp_name = 'quick_buy' )AS cx_experiment_fact on a.checkout_id = cx_experiment_fact.checkout_id
...
# MAGIC %md
# MAGIC ## Payment IDs where UPI SR failed

# COMMAND ----------

raw_data_df[eligibility_condition][(raw_data_df['internal_error_code']=='BAD_REQUEST_PAYMENT_TIMED_OUT')
                                   & (raw_data_df['payment_method']=='upi')
                                    & (raw_data_df['v2_result']=='variant_on')
                                   ][['payment_attempts','producer_created_date','merchant_id','error_code','gateway','upi_meta_app',	'upi_provider',	'upi_app','instrument_name']].head(20)
...
overall_df['cr_contribution'] = overall_df['split_percentage'] * overall_df['overall_cr']
overall_df[overall_df.columns[2:-5]] = overall_df[overall_df.columns[2:-5]].applymap(formatINR)
overall_df[overall_df.columns[-5:]] = overall_df[overall_df.columns[-5:]].applymap(percentage_conversion)
...
# MAGIC %md
# MAGIC #---- FUNNEL ANALYSIS BEGINS HERE [RUN ALL ABOVE] --------------
...
"""
with change_method_event as (
select get_json_object(context,'$.checkout_id') as checkout_id
FROM  events.lumberjack_intermediate 
WHERE 
producer_created_date between '{0}' and '{1}'
and event_name = 'change_method_clicked'
)
...
cx_experiment_fact.flag as exp_flag,
 is_quickbuy_flow,
  ---  COUNT(DISTINCT CASE WHEN (checkout_fact_emr.open  = 1) THEN checkout_fact_emr.checkout_id ELSE NULL END) AS checkout_open,
    ...
    COUNT(DISTINCT CASE WHEN (change_method_event.checkout_id  is not null) THEN checkout_fact_emr.checkout_id ELSE NULL END) AS change_method_clicked,
...
Left join (
       ...
       aggregate_pa.cx_experiment_fact 
       where producer_created_date BETWEEN '{0}' and '{1}'
    and exp_name = 'quick_buy' )AS cx_experiment_fact on checkout_fact_emr.checkout_id = cx_experiment_fact.checkout_id
LEFT JOIN change_method_event ON checkout_fact_emr.checkout_id = change_method_event.checkout_id
...
# MAGIC where 
# MAGIC producer_created_date ='2024-09-24'
# MAGIC
...
funnel_transposed = funnel_transposed[1:]
funnel_transposed

# COMMAND ----------


funnel_transposed['variant_on%'] =funnel_transposed['variant_on']*1.0/ funnel_transposed.iloc[0]['variant_on']
funnel_transposed['control%'] =funnel_transposed['control']*1.0/ funnel_transposed.iloc[0]['control']
funnel_transposed['diff'] = funnel_transposed['variant_on%'] - funnel_transposed['control%']
funnel_transposed.iloc[:,-3:] = funnel_transposed.iloc[:,-3:].applymap(percentage_conversion)
funnel_transposed

# COMMAND ----------

html_code += """<p><h3>Quickbuy High Level Funnel:</h3> 
...
# MAGIC %md
# MAGIC #---------- FUNNEL ANALYSIS ENDS HERE [RUN ALL BELOW] -----------------
...
"""
email_id = ['aravinth.pk@razorpay.com','pallavi.samodia@razorpay.com','revanth.m@razorpay.com','pranav.gupta@razorpay.com','kunal.vishnoi@razorpay.com']
# 
#for i in email_id:
#email_id=['pallavi.samodia@razorpay.com','sanjay.garg@razorpay.com','aakash.bhattacharjee@razorpay.com','manjeet.singh@razorpay.com',]
...
# MAGIC get_json_object(value,'$.shipping_fee' ) as shipping_fee,
# MAGIC get_json_object(value,'$.cod_fee' ) as cod_fee
# MAGIC from realtime_hudi_api.order_meta
# MAGIC where created_date='2024-10-22'
# MAGIC and order_id in (
# MAGIC select order_id from aggregate_pa.magic_checkout_fact
# MAGIC where producer_created_date = date('2024-10-22')
# MAGIC and checkout_id in (
# MAGIC     'PBq9zbXoHmUBWC',
# MAGIC 'PBygQxYEnA2Fhn',
# MAGIC 'PBpQ2XSEdaYmGx',
# MAGIC 'PC9Zt2jbmoEAmi',
# MAGIC 'PBvjkNQC2oPcHh',
# MAGIC 'PC5pEmmkKkhbfl',
# MAGIC 'PBv6G6RVMcyNzS',
# MAGIC 'PBxe0fA1mS3PUy',
# MAGIC 'PC8j4T0rVQ8yQ4',
# MAGIC 'PC94Ccz9jyfOhx',
# MAGIC 'PC6v2fgubeViSP',
# MAGIC 'PC0Ku6SVGYRi0i',
# MAGIC 'PBq6tncYveRqtr',
# MAGIC 'PBuJoF0cDIOnlb',
# MAGIC 'PC5MR3oTy4uBNi',
# MAGIC 'PC79Ch7QDueU8X',
# MAGIC 'PC0HPWCrLMMwF0',
# MAGIC 'PC3H0W1P3epwnk',
# MAGIC 'PC8pQRMmdEclJ3',
# MAGIC 'PBvyUkGD7uNxdf',
# MAGIC 'PBuPrnW8qOq4xL',
# MAGIC 'PC7I8QSX0wUboF',
# MAGIC 'PC0le88OYYOYKX',
# MAGIC 'PC8vKQ6fRWphOO',
# MAGIC 'PC9Rb8cpmYVJKn',
# MAGIC 'PBwVK3cVbEZAMk',
# MAGIC 'PBwideOLU83E5o',
# MAGIC 'PC6WuzP9XMn7vX',
# MAGIC 'PC2H5C0nBm1Uxx',
# MAGIC 'PC6vK2TIFUhMip',
# MAGIC 'PBy4SN6yHm7aD1',
# MAGIC 'PBwRZYkIj2VWVA',
# MAGIC 'PC9KNnz2lEN53A',
# MAGIC 'PCAcJt6i1mIjh9',
# MAGIC 'PBss32x77AGpxa',
# MAGIC 'PByzlHue7yvABF',
# MAGIC 'PByiiS5MictBZn',
# MAGIC 'PBuOmP8hAOHN2C',
# MAGIC 'PC38p64Qc4lb02',
# MAGIC 'PBzkbfR1sGiBhV',
# MAGIC 'PBzkbfR1sGiBhV',
# MAGIC 'PC25KkSfs8NXM2'
# MAGIC )
# MAGIC
# MAGIC )

...
mid = 'Std AWT Sept 24'
Pre_Period_Starting_Date = '2024-09-09'
Pre_Period_Ending_Date='2024-09-15'
Post_Period_Starting_Date='2024-09-16'
Post_Period_Ending_Date='2024-09-22'
...
select p.id, p.method, p.authorized_at, checkout_id from realtime_hudi_api.payments p
  inner join (select checkout_id, payment_id
              from realtime_hudi_api.payment_analytics 
              where ((created_date  between date('{1}')
    and date('{2}'))
     or (created_date  between date('{3}')
    and date('{4}')))
              )pa on p.id = pa.payment_id
  where ((p.created_date  between date('{1}')
    and date('{2}'))
     or (p.created_date  between date('{3}')
    and date('{4}')))
)
SELECT
  ---  a.checkout_id,
 ...
 (get_json_object(render_properties,'$.data.meta.v2_eligible')) as v2_eligible,
  (get_json_object(render_properties, '$.data.meta.v2_result')) AS v2_result,
 merchant_id,   
 select_section,
   select_method,
    a.checkout_id as opens,
   case when method='cod' or authorized_at is not null then payments_cte.checkout_id else null end as submits
--browser_name,

--library,
-- count(distinct a.checkout_id) as opens,
--count(distinct case when method='cod' or authorized_at is not null then payments_cte.checkout_id else null end) as submits
  
  FROM
    aggregate_pa.cx_lo_fact_ism_v1 a
    left join payments_cte on a.checkout_id = payments_cte.checkout_id
  WHERE
    (producer_created_date  between ('{1}')
    and '{2}')
     or (producer_created_date  between '{3}'
    and '{4}')
...
base_int_table = base_db.toPandas()
base_int_table.head()

# COMMAND ----------

base_table = base_int_table.groupby(by=['mnth','integration_type','v2_eligible','v2_result','merchant_id','select_section','select_method','platform',]).agg({'opens':'nunique','submits':'nunique'}).reset_index()
...
base_temp.rename(columns={'opens':'open', 'submits':'submit',},inplace=True)
base_temp
...
html_code += (volume_impact_df[[volume_impact_feature,'Pre Volume %','Post Volume %','Volume Impact',]].head(50)).to_html(index=False,escape=False,  )
html_code +="""</p>"""
...
for index, row in impact_consolidated_filtered_df.head(num_factors).iterrows():
    ...
    html_code += (combination_impact_df.head(50)).to_html(index=False, escape=False, )
    html_code += """</br></br></p>"""  
    html_code += """</li>"""
...
email_id = ['sanjay.garg@razorpay.com','pallavi.samodia@razorpay.com',]
for i in email_id:
